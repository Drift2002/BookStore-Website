...
const Hero = ({searchQuery}) => {
    ...
    useEffect(() => {
        const fetchBooks = async() =>{
            try{
                setLoading(true);
                const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(searchQuery)}&maxResults=35&key=YOURAPIKEY`)
    ...
    const getPlaceholder = (title) => {
        const initials =title.split(" ").slice(0,3).map(word => word[0]?.toUpperCase() || "").join('');
        ...
        return (
            <div className=" min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-8 flex justify-center items-center">
                <p className=" text-cyan-300 text-lg md:text-xl text-center">
                    No matching books for your search ... !
                </p>
            </div>
        )
    }
            ...
            <div className=" max-w-7xl mx-auto">
                        ...
                        if(book.dummy) {
                            return (
                                <div key={book.id || index} className=" group relative bg-gray-800/30 rounded-xl md:rounded-2xl backdrop-blur-xl border border-dashed border-gray-700/50 flex items-center justify-center h-64 md:h-96">
                                    <p className=" text-gray-400 text-sm md:text-base">No book found</p>
                                </div>
                            )
                        }
                        return (
                ...
                <div className=" mt-6 md:mt-8 flex flex-wrap justify-center items-center gap-2 md:gap-3">
                    <button onClick={() => setCurrentPage(p => Math.max(1, p-1))} disabled = {currentPage === 1}
                        className=" px-3 py-1 md:px4 md:py-2 rounded-md md:rounded-lg bg-gray-900/80 border border-gray-700/50 text-cyan-300 text-xs md:text-sm hover:bg-gray-900/60 transition-colors disabled:opacity-50">
                        Previous
                    </button>
